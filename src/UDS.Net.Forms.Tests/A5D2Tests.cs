...
namespace UDS.Net.Forms.Tests
{
    ...
    public class A5D2Tests : TestBase
    {
        ...
        [TestMethod]
        public async Task A5D2_Validate_Against_Age_At_Visit()
        {
            //Add birth year to visit via the A1 form
            await Page.GotoAsync(BaseUrl);
            await Page.GetByRole(AriaRole.Button, new() { Name = "Go" }).ClickAsync();
            await Expect(Page.GetByRole(AriaRole.List)).ToContainTextAsync("A1");
            await Page.GetByRole(AriaRole.Link, new() { Name = "A1 Participant Demographics", Exact = true }).ClickAsync();
            await Page.Locator("input[name=\"A1.BIRTHMO\"]").FillAsync("01");
            ...
            //Update age fields to a value higher than the age provided in the A1. This should trigger validation when the form posts
            foreach (var field in AgeRelatedQuestions)
            {
                await Page.Locator($"input[name=\"A5D2.{field}\"]").FillAsync("85");
        ...
        private async Task WriteInitialVisitFormData()
        {
            // Writes data to all fields on form
            await Page.Locator("input[name=\"A5D2.TOBAC100\"][value=\"1\"]").CheckAsync();
            await Page.Locator("input[name=\"A5D2.SMOKYRS\"]").FillAsync("50");
